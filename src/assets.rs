...
pub struct AssetLedger {
    coins: HashMap<String, CoinDefinition>,
...
impl AssetOperation {
    ...
    pub fn requires_zero_xyqon_amount(&self) -> bool {
        match self {
            AssetOperation::CreateCoin(_) | AssetOperation::TransferCoin(_) => true,
        }
    }
}
...
impl AssetLedger {
    ...
    pub fn coin_exists(&self, symbol: &str) -> bool {
        self.coins.contains_key(&symbol.to_ascii_uppercase())
    }
    ...
    fn create_coin(
        ...
        let name = normalize_name(&coin.name)?;
        ...
        let definition = CoinDefinition {
            symbol: symbol.clone(),
            name,
            owner_public_key: owner_public_key.clone(),
            initial_supply: coin.supply,
        };
        self.coins.insert(symbol.clone(), definition);
